...
class Bhush extends Component {
    ...
    componentDidMount(){
        getToken().then(token=>{
            ...
            SalesService.getBhushSales(token).then(res=>{
                let tableData =[]
    ...
    componentWillReceiveProps(){
        this.setState({loading:true})
        SalesService.getBhushSales(this.state.token).then(res=>{
            let tableData =[]
            res.reverse().map((datum,i)=>{
                tableData.push([i+1,datum.customer,datum.unitPrice,datum.noofPackets,datum.date,datum.total])
            })
    ...
    isValid(){
        ...
        if(state.customer && state.unitPrice && state.noofPackets ) return true;
        else return false;
    }
    ...
    onSubmit=()=>{
        ...
        Keyboard.dismiss();
        const state = this.state;
        SalesService.postBhushSales(state.token,state.customer,state.unitPrice,state.noofPackets,state.date).then(res=>{
    ...
    render() {
        ...
        return (
           <View style={styles.container}>
                <Header title="Bhush Sales" navigation={this.props.navigation}/>
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled">
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Add Bhush Sales</Text>
