...
class Kanika extends Component {
    ...
    componentDidMount(){
        getToken().then(token=>{
            ...
            SalesService.getKanikaSales(token).then(res=>{
                let tableData =[]
    ...
    componentWillReceiveProps(){
        this.setState({loading:true})
        SalesService.getKanikaSales(this.state.token).then(res=>{
            let tableData =[]
            res.reverse().map((datum,i)=>{
                tableData.push([i+1,datum.customer,datum.unitPrice,datum.noofPackets,datum.date,datum.total])
            })
    ...
    onSubmit=()=>{
        ...
        Keyboard.dismiss();
        const state = this.state;
        SalesService.postKanikaSales(this.state.token,state.customer,state.unitPrice,state.noofPackets,state.date).then(res=>{
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled">
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Add Kanika Sales</Text>
