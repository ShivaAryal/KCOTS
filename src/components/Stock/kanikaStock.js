...
class Kanika extends Component {
    ...
    componentDidMount(){
        getToken().then(token=>{
            ...
            StockService.getKanikaStock(token).then(res=>{
                let tableData =[]
                res.map((datum,i)=>{
    ...
    onSubmit(){
        ...
        StockService.postKanikaStock(this.state.token,good,unitPacketPrice,noofPackets).then(res=>{
            alert("Succesfully added")
            StockService.getKanikaStock(this.state.token).then(res=>{
            let tableData =[]
            res.map((datum,i)=>{
                tableData.push([i+1,datum.good,datum.unitPrice,datum.noofPackets])
            })
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled" >
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Add Kanika Stock</Text>
                            <FAB
                                icon="close"
                                color="#fff"
                        ...
                        <Picker
                            ...
                            mode="dropdown">
                            <Picker.Item value="Masino Kanika" label="Masino Kanika" />
                            <Picker.Item value="Moto Kanika" label="Moto Kanika" />
                        </Picker>
