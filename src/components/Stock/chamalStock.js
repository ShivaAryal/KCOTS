...
import { View, StyleSheet,Picker,ScrollView, Modal, Text, TouchableOpacity, KeyboardAvoidingView, ActivityIndicator, Keyboard } from 'react-native';
...
class Chamal extends Component {
    ...
    componentDidMount(){
        getToken().then(token=>{
            ...
            StockService.getChamalStock(token).then(res=>{
                let tableData =[]
                res.map((datum,i)=>{
    ...
    onSubmit(){
        ...
        StockService.postChamalStock(this.state.token,good,unitPacketPrice,noofPackets).then(res=>{
            alert("Succesfully added")
            StockService.getChamalStock(this.state.token).then(res=>{
            let tableData =[]
            res.map((datum,i)=>{
                tableData.push([i+1,datum.good,datum.unitPrice,datum.noofPackets])
            })
    ...
    render() {
        ...
        return (
           <View style={styles.container}>
                <Header title="Chamal Stock" navigation={this.props.navigation}/>
                ...
                <ScrollView contentContainerStyle={styles.modalBackground} keyboardShouldPersistTaps="handled" >
                    <View style={{backgroundColor:'#fff',padding:10,borderRadius:10}}>
                        <View style={{flexDirection:'row',justifyContent:'space-between'}}> 
                            <Text style={{fontSize:20,marginLeft:10,marginVertical:5}}>Add Chamal Stock</Text>
                            <FAB
                                icon="close"
                                color="#fff"
                        ...
                        <Picker
                            ...
                            mode="dropdown">
                            <Picker.Item value="Masino Chamal" label="Masino Chamal" />
                            <Picker.Item value="Moto Chamal" label="Moto Chamal" />
                        </Picker>
